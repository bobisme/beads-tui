...
kitty --title "beads-tui-screenshot" -e bash -c "./target/release/bu; read" &
KITTY_PID=$!
...
hyprctl dispatch focuswindow "title:beads-tui-screenshot" >/dev/null
...
GEOMETRY=$(hyprctl clients -j | jq -r '.[] | select(.title == "beads-tui-screenshot") | "\(.at[0]),\(.at[1]) \(.size[0])x\(.size[1])"')
...
grim -g "$GEOMETRY" /tmp/beads-tui-auto.png
...
pngquant --force --output "$OUTPUT" /tmp/beads-tui-auto.png
